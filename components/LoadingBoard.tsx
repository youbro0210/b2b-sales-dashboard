...
export default function LoadingBoard({
  ...
  title: string;
  groups: string[]; // 이 화면에서 다룰 채널 그룹 (기준정보의 '구분')
}) {
  ...
  const removeRange = async () => {
    ...
    try {
      ...
      const n = await countLoadingRange(delFrom, delTo, ids);
      if (!n) {
        setDelMsg(`${delFrom} ~ ${delTo} 기간에 저장된 데이터가 없습니다.`);
      } else if (
        confirm(
          `${delFrom} ~ ${delTo} 기간의 ${title} 데이터 ${n}건을 삭제합니다.\n삭제하면 되돌릴 수 없습니다. 진행할까요?`
        )
      ) {
        await deleteLoadingRange(delFrom, delTo, ids);
        setDelMsg(`${n}건 삭제 완료`);
        fetchValues();
      }
    } catch (e: any) {
  ...
  useEffect(() => {
    listChannels().then((d) => {
      ...
      // group_name 은 그룹의 첫 채널에만 들어있고 이후는 null 이므로 앞의 값을 이어받는다
      let cur = "";
      const withGroup = all.map((c) => {
        if (c.group_name) cur = c.group_name;
        return { ...c, group_name: c.group_name || cur };
      });
      setChannels(withGroup.filter((c) => groups.includes(c.group_name || "")));
    });
  }, [groups]);
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-bold">{title} 입력</h1>
      ...
      <div className="card">
        ...
        <p className="text-[11px] text-slate-400 mt-2">
          선택한 기간의 <b>{title}</b> 데이터만 삭제합니다(다른 메뉴 데이터는 유지). 삭제 전 건수를 확인시켜 드리며, 되돌릴 수 없습니다.
        </p>
      </div>
