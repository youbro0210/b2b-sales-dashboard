...
// B2C 오프라인 / B2C 온라인 입력: 일별 등록 (날짜 선택 → 채널별 금액 입력)
export default function LoadingBoard({
  ...
  useEffect(() => {
    listChannels().then((d) => {
      ...
      setChannels(
        wg.filter((c) => groups.includes(c.group_name || "") && !isSum(c.name))
      );
    });
    // groups 는 페이지에서 고정 배열이라 최초 1회만 조회한다
      ...
      <div className="card overflow-x-auto">
        {loading ? (
          ...
          <p className="text-center text-slate-400 py-6">
            등록된 채널이 없습니다. 기준정보 관리 → 상차 채널에서 추가하세요.
          </p>
        ) : (
