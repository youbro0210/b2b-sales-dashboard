...
import { monthKST, todayKST } from "@/lib/types";

// 현황 화면 공통 껍데기: 제목 + 기간(시작~종료) 조회 + 엑셀 다운로드 버튼.
// 실제 표/합계는 children 으로 각 현황이 채운다.
export default function ReportShell({
  ...
  const preset = (kind: "month" | "prevMonth" | "today") => {
    const t = todayKST();
    if (kind === "today") {
      setFrom(t);
      setTo(t);
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-500">기간별 조회 · 조회 전용(수정 불가)</p>
      </div>
